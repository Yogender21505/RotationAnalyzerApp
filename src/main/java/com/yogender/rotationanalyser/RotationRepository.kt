...
class RotationRepository(private val rotationDao:RotationDAO) {
    ...
    suspend fun getListofDatabase(
        databaseList: MutableList<Rotation>,
        ...
        withContext(Dispatchers.IO) {
            ...
            var progress = 0f
            rotationsFromDB.forEachIndexed { index, rotation ->
                databaseList.add(rotation)
                progress = (index + 1).toFloat() / rotationsFromDB.size * 100
                loadingProgress.value = progress
            }
            println(loadingProgress.value)
